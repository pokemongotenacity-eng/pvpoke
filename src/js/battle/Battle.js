...
function Battle() {
	...
	this.getBattleExportData = function () {
		return {
			pokemon: pokemon.map(function (p) {
				return {
					...
					shields: p.shields,
					startHp: p.startHp,
					startEnergy: p.startEnergy,
					startingShields: p.startingShields,
					fastMove: p.fastMove ? p.fastMove.name : "",
					chargedMoves: p.chargedMoves.map(function (m) {
						return m.name;
					})
				};
			}),
			...
			duration: duration,
			displayTime: matchupDisplayTime,
			timeline: timeline
		};
	}
